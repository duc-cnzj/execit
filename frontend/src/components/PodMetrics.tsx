...
const PodMetrics: React.FC<{ namespace: string; pod: string; timestamp: any, clusterId: number }> =
  ({ namespace, pod, timestamp, clusterId }) => {
    ...
    let { close } = useStream(
      `${process.env.REACT_APP_BASE_URL}/api/metrics/clusters/${clusterId}/namespace/${namespace}/pods/${pod}/stream?time=${timestamp}`,
      {
        onNext,
        onError,
