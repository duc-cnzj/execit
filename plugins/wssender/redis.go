...
func (p *redisSender) Initialize(args map[string]any) error {
	...
	p.rds = rdb
	xlog.Info("[Plugin]: " + p.Name() + " plugin Initialize...")
	return nil
}

func (p *redisSender) Destroy() error {
	p.rds.Close()
	xlog.Info("[Plugin]: " + p.Name() + " plugin Destroy...")
	return nil
}
...
func (p *rdsPubSub) Close() error {
	xlog.Debugf("[Websocket]: Closed, uid: %v id: %v", p.uid, p.id)
	close(p.close)
	return nil
}
...
func (p *rdsPubSub) Subscribe() <-chan []byte {
	...
	channel := ps.Channel()
	xlog.Debugf("[Websocket]: Subscribe Start id: %v channels: %v %s", p.id, p.id, BroadcastRoom)
	go func() {
		for {
			select {
			case msg, ok := <-channel:
			...
			case <-p.close:
				ps.Close()
				xlog.Debugf("[Websocket]: redis channel closed, uid: %s, id: %v", p.uid, p.id)
				return
			}
		}
	}()
